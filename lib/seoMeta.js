...
const pctTaxaMp = Math.round(TAXA_PLATAFORMA * 100);
const pctTaxaWoovi = Math.round(TAXA_PLATAFORMA_WOOVI * 100);
const pctOrgMp = Math.round(ORGANIZADOR_PERCENTUAL * 100);
const taxaFixaWoovi = TAXA_FIXA_COTA_WOOVI.toFixed(2).replace('.', ',');
const taxaWooviLabel = `${pctTaxaWoovi}% + R$ ${taxaFixaWoovi}/cota`;

const TEXTO_COMISSAO = `Comissão de ${pctTaxaMp}% no Mercado Pago (organizador recebe ${pctOrgMp}%) ou ${taxaWooviLabel} no PIX via plataforma. O comprador paga o valor exato da cota, sem acréscimo.`;
...
function extractWords(text, max = 6) {
  if (!text) return [];
  const stopWords = new Set([
    'de','do','da','dos','das','e','em','no','na','nos','nas','o','a','os','as',
    'um','uma','para','por','com','que','se','ao','à','mais','mas','ou','seu','sua'
  ]);
  return String(text)
    .replace(/<[^>]+>/g, '')
    .toLowerCase()
...
function tenantKeywords(tenantOrName) {
  const nome = typeof tenantOrName === 'string' ? tenantOrName : tenantOrName?.nome || '';
  const descricao = typeof tenantOrName === 'object' ? tenantOrName?.descricao : null;
  const descWords = extractWords(descricao, 5);
...
function rifaKeywords(rifaOrTitulo, tenantName) {
  const titulo = typeof rifaOrTitulo === 'string' ? rifaOrTitulo : rifaOrTitulo?.titulo || '';
  const nome = tenantName || '';

  const descricao = typeof rifaOrTitulo === 'object' ? rifaOrTitulo?.descricao : null;
  const premios = typeof rifaOrTitulo === 'object' && Array.isArray(rifaOrTitulo?.premios)
    ? rifaOrTitulo.premios.map((p) => p.titulo).filter(Boolean)
    : [];

  const descWords = extractWords(descricao, 5);
  const premioWords = premios.slice(0, 3);
...
function platformFaq() {
  return [
    ...
    },
    {
      ...
      a: `Sim. Não há mensalidade para criar conta nem limite de rifas. ${TEXTO_COMISSAO}`
    },
    {
      q: 'Qual a comissão da VouRifar?',
      a: TEXTO_COMISSAO
    },
    {
...
function platformLandingMeta(appUrl) {
  ...
  return {
    ...
    seoDescription: truncate(
      `Plataforma grátis para criar rifas online. PIX automático, cotas ilimitadas e comissão de ${pctTaxaMp}% (Mercado Pago) ou ${taxaWooviLabel} (PIX plataforma). Comprador paga valor exato da cota.`
    ),
    ...
    seoJsonLd: {
      ...
      '@graph': [
        ...
        },
        {
          ...
          url,
          description: `Crie rifas online com PIX automático. Grátis para começar — comissão de ${pctTaxaMp}% (MP) ou ${taxaWooviLabel} (PIX plataforma) sobre vendas, sem mensalidade.`,
        ...
        },
        {
          ...
          offers: {
            ...
            price: '0',
            priceCurrency: 'BRL',
            description: `Gratuito para criar conta. Comissão de ${pctTaxaMp}% (MP) ou ${taxaWooviLabel} (PIX plataforma) sobre vendas`
          },
          featureList: [
            ...
            'Sem mensalidade',
            `Comissão ${pctTaxaMp}% Mercado Pago ou ${taxaWooviLabel} PIX plataforma`
          ]
        },
        faqJsonLd(faq)
      ]
...
function cadastroMeta(appUrl) {
  ...
  const image = absUrl(appUrl, DEFAULT_OG_IMAGE);
  const faq = platformFaq().filter((item) =>
    item.q.includes('comissão') || item.q.includes('gratuita')
  );

  return {
    seoTitle: 'Criar Sistema de Rifas Grátis — Cadastro VouRifar',
    seoDescription: truncate(
      `Cadastre-se grátis e crie rifas online em minutos. PIX automático, comissão de ${pctTaxaMp}% (MP) ou ${taxaWooviLabel} (PIX plataforma). Comprador paga valor exato da cota.`
    ),
      ...
      '@graph': [
        {
          ...
          url,
          description: `Crie sua conta grátis na VouRifar. ${TEXTO_COMISSAO}`,
          potentialAction: {
            '@type': 'RegisterAction',
            target: url,
...
function tenantIndexMeta({ baseUrl, tenant }) {
  ...
  return {
    ...
    seoKeywords: tenantKeywords(tenant),
    seoUrl: tenantUrl,
    seoType: 'website',
...
function rifaDetalheMeta({ baseUrl, tenant, rifa }) {
  ...
  return {
    ...
    seoKeywords: rifaKeywords(rifa, tenant.nome),
    seoUrl: rifaUrl,
    seoType: 'product',
    seoImage: rifaImage,
