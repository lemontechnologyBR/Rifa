...
/* ================================================================
   ...
   ================================================================ */
function templateVendaOrganizador({ rifa, reserva, usuario, numeros, tenantSlug, organizadorPercentual }) {
  const pct = organizadorPercentual || 0.93;
  const valorOrganizador = Number(reserva.valorTotal) * pct;
  ...
  const conteudo = `
    ...
    ${caixaDestaque(`
      ...
      <p style="margin:0;font-size:28px;font-weight:900;color:#0f172a;">R$ ${valorOrganizador.toFixed(2).replace('.', ',')}</p>
      <p style="margin:4px 0 0;font-size:12px;color:#64748b;">${Math.round(pct * 100)}% de R$ ${Number(reserva.valorTotal).toFixed(2).replace('.', ',')}</p>
    `, '#f0fdf4', '#10b981')}
...
/* ================================================================
   TEMPLATE 13 — Aviso Mercado Pago desativado temporariamente
   ================================================================ */
function templateAvisoMpDesativado({ organizador, tenantSlug, temPix }) {
  const carteiraUrl = `${APP_URL}/${tenantSlug}/admin/carteira`;

  const conteudo = `
    <p style="margin:0 0 6px;font-size:22px;font-weight:800;color:#0f172a;">Importante: mudança no recebimento</p>
    <p style="margin:0 0 24px;font-size:15px;color:#475569;">
      Olá, ${organizador.nome.split(' ')[0]}! Precisamos avisar sobre uma alteração urgente na sua conta VouRifar.
    </p>

    ${badge('AVISO', '#f59e0b', 'rgba(245,158,11,0.1)')}

    ${caixaDestaque(`
      <p style="margin:0 0 10px;font-size:14px;font-weight:700;color:#0f172a;">Por que o Mercado Pago foi desativado?</p>
      <p style="margin:0;font-size:13px;color:#475569;line-height:1.7;">
        O Mercado Pago enviou um aviso oficial indicando que o uso da conta em atividades de sorteios pode ser considerado produto proibido nos Termos deles.
        Para <strong>proteger sua conta Mercado Pago</strong> (e evitar suspensão), desativamos temporariamente essa modalidade na VouRifar.
      </p>
    `, '#fff7ed', '#f59e0b')}

    ${caixaDestaque(`
      <p style="margin:0 0 8px;font-size:14px;font-weight:700;color:#0f172a;">Como você continua vendendo</p>
      <ul style="margin:0;padding-left:20px;font-size:13px;color:#475569;line-height:1.8;">
        <li>Receba via <strong>chave PIX</strong> na Carteira da VouRifar</li>
        <li>Taxa da plataforma reduzida para <strong>5%</strong> — você fica com <strong>95%</strong></li>
        <li>Saque o saldo quando quiser pelo painel</li>
      </ul>
    `)}

    ${temPix
      ? `<p style="margin:16px 0 0;font-size:14px;color:#475569;">Você já tem chave PIX cadastrada — suas novas vendas já seguem pela modalidade plataforma.</p>`
      : `<p style="margin:16px 0 0;font-size:14px;color:#b45309;font-weight:600;">Atenção: você ainda não cadastrou chave PIX. Sem ela, os compradores não conseguem pagar. Configure agora na Carteira.</p>`
    }

    ${btnVerde(temPix ? 'Abrir minha carteira' : 'Cadastrar chave PIX agora', carteiraUrl)}
  `;

  return baseLayout({
    titulo: 'Mudança no Mercado Pago',
    conteudo,
    rodapeExtra: 'Qualquer dúvida, responda este e-mail. Equipe VouRifar.'
  });
}
