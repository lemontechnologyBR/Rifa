...
/* ================================================================
   TEMPLATE 5b — Confirmação de e-mail (organizador)
   ================================================================ */
function templateConfirmacaoEmail({ organizador, token }) {
  const confirmUrl = `${APP_URL}/confirmar-email?token=${token}`;

  const conteudo = `
    <p style="margin:0 0 6px;font-size:22px;font-weight:800;color:#0f172a;">Confirme seu e-mail</p>
    <p style="margin:0 0 24px;font-size:15px;color:#475569;">
      Olá, ${organizador.nome}! Para ativar sua conta na VouRifar, confirme que este e-mail é seu.
    </p>

    ${caixaDestaque(`
      <p style="margin:0 0 4px;font-size:13px;font-weight:700;color:#0f172a;">Este link expira em <strong>48 horas</strong>.</p>
      <p style="margin:0;font-size:13px;color:#475569;">Se você não criou uma conta, ignore este e-mail.</p>
    `, '#fafafa', '#e2e8f0')}

    ${btnVerde('Confirmar meu e-mail', confirmUrl)}

    <p style="font-size:12px;color:#94a3b8;text-align:center;margin-top:16px;">
      Se o botão não funcionar, copie o link abaixo:<br>
      <a href="${confirmUrl}" style="color:#10b981;font-size:11px;word-break:break-all;">${confirmUrl}</a>
    </p>
  `;
  ...
  return baseLayout({
    ...
    conteudo,
    rodapeExtra: 'Este e-mail foi enviado porque alguém usou este endereço no cadastro da VouRifar.'
  });
}
  ...
  const conteudo = `
    ...
    ${caixaDestaque(`
      ...
      <ol style="margin:0;padding-left:20px;font-size:13px;color:#475569;line-height:1.8;">
        <li>Crie sua primeira rifa (leva poucos minutos)</li>
        <li>Configure sua chave PIX na Carteira para receber</li>
        <li>Compartilhe o link com seus clientes</li>
      </ol>
    `, '#f0fdf4', '#10b981')}

    ${btnVerde('Criar minha primeira rifa', `${adminUrl}/rifas?nova=1`)}
  `;
...
function templatePrimeiraRifaD1({ organizador, tenantSlug, carteiraOk }) {
  ...
  const btnTexto = 'Criar minha primeira rifa';

  const conteudo = `
    <p style="margin:0 0 6px;font-size:22px;font-weight:800;color:#0f172a;">Sua loja está pronta! 🎯</p>
    <p style="margin:0 0 24px;font-size:15px;color:#475569;">
      Olá, ${organizador.nome.split(' ')[0]}! Você criou sua conta na VouRifar ontem — falta só publicar o primeiro sorteio.
    </p>
    ...
    ${caixaDestaque(`
      ...
      <ul style="margin:0;padding-left:20px;font-size:13px;color:#475569;line-height:1.8;">
        ...
        <li>Depois configurar o PIX na Carteira para receber</li>
      </ul>
    `, '#eff6ff', '#3b82f6')}
...
function templatePrimeiraRifaD3({ organizador, tenantSlug, carteiraOk }) {
  const criarUrl = `${APP_URL}/${tenantSlug}/admin/rifas?nova=1`;
  const btnTexto = 'Criar sorteio agora';
  ...
  const conteudo = `
    ...
    ${caixaDestaque(`
      <p style="margin:0;font-size:14px;color:#475569;line-height:1.7;">
        <strong style="color:#0f172a;">Dica:</strong> comece simples — um prêmio, cotas a partir de R$ 5. O PIX na Carteira você configura quando for receber.
      </p>
    `, '#fffbeb', '#f59e0b')}
...
/* ================================================================
   TEMPLATE 11b — Nurture D+7 — reativação cadastro sem rifa
   ================================================================ */
function templatePrimeiraRifaD7({ organizador, tenantSlug, carteiraOk }) {
  const criarUrl = `${APP_URL}/${tenantSlug}/admin/rifas?nova=1`;
  const btnTexto = 'Publicar meu sorteio agora';

  const conteudo = `
    <p style="margin:0 0 6px;font-size:22px;font-weight:800;color:#0f172a;">Sua conta ainda está ativa 💡</p>
    <p style="margin:0 0 24px;font-size:15px;color:#475569;">
      ${organizador.nome.split(' ')[0]}, faz uma semana que você se cadastrou na VouRifar e ainda não publicou um sorteio.
      Organizadores que criam a primeira rifa costumam vender nas primeiras horas — o link é o que falta.
    </p>

    ${badge('REATIVAÇÃO', '#8b5cf6', 'rgba(139,92,246,0.1)')}

    ${caixaDestaque(`
      <p style="margin:0 0 8px;font-size:14px;font-weight:700;color:#0f172a;">Comece simples hoje:</p>
      <ul style="margin:0;padding-left:20px;font-size:13px;color:#475569;line-height:1.8;">
        <li>Um prêmio claro + cotas a partir de R$ 5</li>
        <li>Publique em menos de 5 minutos</li>
        <li>Configure o PIX depois, só para receber</li>
      </ul>
    `, '#f5f3ff', '#8b5cf6')}
  ...
  return baseLayout({
    titulo: 'Ainda dá tempo de vender',
    conteudo,
    rodapeExtra: 'Se não for o momento, ignore este e-mail — sua conta continua disponível.'
  });
}
...
function templateAtualizacaoPainelRifas({ organizador, tenantSlug, rifasAtivas = [] }) {
  const lojaUrl = `${APP_URL}/${tenantSlug}`;
  const adminUrl = `${APP_URL}/${tenantSlug}/admin`;
  const rifasUrl = `${APP_URL}/${tenantSlug}/admin/rifas`;
  const nome = organizador.nome.split(' ')[0];
  ...
  const conteudo = `
    <p style="margin:0 0 6px;font-size:22px;font-weight:800;color:#0f172a;">Melhorias no painel das rifas 🎟️</p>
    <p style="margin:0 0 24px;font-size:15px;color:#475569;">
      Olá, ${nome}! Liberamos ajustes práticos para você gerenciar sorteios ativos com mais liberdade.
    </p>

    ${badge('ATUALIZAÇÃO', '#10b981', 'rgba(16,185,129,0.1)')}

    ${caixaDestaque(`
      <p style="margin:0 0 10px;font-size:14px;font-weight:700;color:#0f172a;">O que mudou no seu painel</p>
      <ul style="margin:0;padding-left:20px;font-size:13px;color:#475569;line-height:1.85;">
        <li><strong style="color:#0f172a;">Editar total de cotas com a rifa ativa</strong> — pode aumentar livremente; para reduzir, só remove cotas ainda disponíveis (não vendidas/reservadas)</li>
        <li><strong style="color:#0f172a;">Excluir sorteio</strong> — botão Excluir na lista e na edição (bloqueado se já houver cotas pagas)</li>
        <li><strong style="color:#0f172a;">Carteira</strong> — lembre da verificação de identidade (KYC) antes do próximo saque</li>
      </ul>
    `)}

    ${caixaDestaque(`
      <p style="margin:0;font-size:14px;color:#475569;line-height:1.7;">
        Abra <strong>Rifas</strong> no painel para editar o total de cotas ou excluir um sorteio que ainda não teve venda paga.
      </p>
      ${listaRifas}
    `, '#eff6ff', '#3b82f6')}
  ...
  return baseLayout({
    titulo: 'Melhorias no painel — VouRifar',
    conteudo,
    rodapeExtra: 'Equipe VouRifar — qualquer dúvida, use o chat de suporte no painel ou responda este e-mail.'
  });
}
